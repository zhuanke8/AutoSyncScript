...
const retainWater = $.isNode() ? (process.env.retainWater ? process.env.retainWater : 0) : ($.getdata('retainWater') ? $.getdata('retainWater') : 0);//保留水滴大于多少g,默认100g;
...
async function jdFruit() {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      await $.get({
        url: 'http://zhuli.ipas.eu.org:8080/activeJdFruitCode?code=' + $.farmInfo.farmUserPro.shareCode
...
async function collect() {
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${$.farmInfo.farmUserPro.shareCode}\n`);
      shareCodeStr += $.farmInfo.farmUserPro.shareCode + '&';
      jdFruitShareArr.push($.farmInfo.farmUserPro.shareCode)
    } else {
      console.log(`初始化农场数据异常, 请登录京东 app查看农场0元水果功能是否正常,农场初始化数据: ${JSON.stringify($.farmInfo)}`);
    }
